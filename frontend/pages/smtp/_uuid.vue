<template>
  <div class="flex flex-col h-full flex-grow py-5 px-4 md:px-6 lg:px-8">
    <div class="flex flex-col flex-reverse md:flex-row justify-between items-center">
      <h2 class="text-sm sm:text-base md:text-lg lg:text-2xl">
        {{ event.subject }}
      </h2>
    ...
    <div class="text-xs font-semibold mt-3 flex flex-wrap items-center">
      ...
      >
        ...
        <div class="px-3 py-1 bg-blue-800 text-white font-semibold rounded-r">
          {{ email.name }} [{{ email.email }}]
        </div>
      </div>
      ...
      >
        ...
        <div class="px-3 py-1 bg-red-800 text-white font-semibold rounded-r">
          {{ email.name }} [{{ email.email }}]
        </div>
      </div>
      ...
      >
        ...
        <div class="px-3 py-1 bg-purple-800 text-white font-semibold rounded-r">
          {{ email.name }} [{{ email.email }}]
        </div>
      </div>
      ...
      >
        ...
        <div class="px-3 py-1 bg-green-800 text-white font-semibold rounded-r">
          {{ email.name }} [{{
            email.email
          }}]
        </div>
    ...
    <Tabs class="flex-1">
      ...
      <Tab title="HTML">
        <CodeSnippet
          language="html"
          class="max-w-full"
          :code="event.event.html"
        />
      </Tab>
      <Tab title="Raw">
        <CodeSnippet
          language="html"
          :code="event.event.raw"
        />
      </Tab>
      <Tab title="Tech Info">
        <div>
          ...
          <Table>
            ...
            <TableRow
              v-if="event.event.cc.length"
              title="Cc"
            >
              <Addresses :addresses="event.event.cc" />
            </TableRow>
            <TableRow
              v-if="event.event.bcc.length"
              title="Bcc"
            >
              <Addresses :addresses="event.event.bcc" />
            </TableRow>
            <TableRow
              v-if="event.event.reply_to.length"
              title="Reply to"
            >
              <Addresses :addresses="event.event.reply_to" />
            </TableRow>
            <TableRow
              v-if="event.event.attachments.length"
              title="Attachments"
            >
